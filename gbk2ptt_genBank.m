function gbk2ptt_genBank(gbk_file)

%take gbk file and convert to .ptt (tab delimited)
    %designed gbk from BAA-835 CP0011071.1 GenBank annotation resource
%.ptt fields: 
%Location / Strand / Length / PID / Gene / Synonym / Code / COG / Product
%793..1353 / - / 561 / WP_012419063.1 / yfgA / Amuc_0001 / Amuc_0001 / - / pooase

%Version _genBank: for use with GenBank resource format (not RefSeq), e.g. CP0011071.1
    %places RS##### where Amuc_#### is absent
...
for i=1:len,
    ...
    for j=1:6,
        if a(j,1:6)=='/locus'
            cod = a(j,13:21); %locus_tag for Code field
            ji=j;
        end
    end
    %if a(ji-1,1:5)=='/gene'
    ...
    %else syn = cod; %if absent, use locus_tag
    %end
gene = [gene; {gen}];
synon = [synon; {cod}];
code = [code; {cod}];
...
outfile = strcat(gbk_file(1:strfind(gbk_file,'.gb')),'ptt');
